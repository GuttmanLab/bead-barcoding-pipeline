"""
Count the number of reads at each stage in the bead-barcoding pipeline.

Dependencies
- samtools
- standard linux tools: ls, xargs, grep, wc, sort, uniq, awk, sh, gzip
- Python libraries: NumPy, Pandas

Notes
- The pipeline order is specified in the global PIPELINE and PIPELINE_CLUSTERS
  variables.
"""

import argparse
import glob
import json
import os
import re
import shutil
import shlex
import subprocess
import sys
import numpy as np
import pandas as pd

regex_uniq_count = re.compile(r"\s*(?P<count>\d+)\s+(?P<pattern>.*)")
regex_fastq = re.compile(r'(\.fastq\.gz$)|(\.fq.gz$)|(\.fastq$)|(\.fq$)')
regex_bam = re.compile(r'(\.bam$)|(\.sam$)|(\.cram$)')

PIPELINE = {
    # (split_fastq) raw reads
    "split_fastq": {"parent": None, "path": os.path.join("split_fastq", "{sample}_R1.part_*.fastq.gz")},
    # (adaptor_trimming_pe) adapter trimming
    "trim": {"parent": "split_fastq", "path": os.path.join("trimmed", "{sample}*val_1.fq.gz")},
    # (barcode_id) barcode idenfication - should not discard any reads; barcode gets appended to read name
    "barcode": {
        "parent": "trim",
        "path": os.path.join("fastqs", "{sample}*_R1*.barcoded.fastq.gz"),
    },
    "unassigned": {
        "parent": "trim",
        "path": os.path.join("fastqs", "{sample}*_R1*.unassigned.fastq.gz"),
    },
    # (split_bpm_dpm, split_bpm_rpm) Split reads into DPM, BPM, RPM
    "bpm": {
        "parent": "barcode",
        "path": os.path.join("fastqs", "{sample}*_R1*.barcoded_bpm.fastq.gz"),
    },
    "dpm": {
        "parent": "barcode",
        "path": os.path.join("fastqs", "{sample}*_R1*.barcoded_dpm.fastq.gz"),
    },
    "rpm": {
        "parent": "barcode",
        "path": os.path.join("fastqs", "{sample}*_R1*.barcoded_rpm.fastq.gz"),
    },
    ## DNA processing ##
    # (cutadapt_dpm) Remove DPM from read1 of DPM reads
    "dpm_trim": {
        "parent": "dpm",
        "base": "dpm",
        "path": os.path.join("trimmed", "{sample}*_R1*.barcoded_dpm.RDtrim.fastq.gz"),
    },
    # (bowtie2_align) Align to genome
    "bowtie2_dna": {
        "parent": "dpm_trim",
        "base": "dpm",
        "path": os.path.join("alignments_parts", "{sample}*.DNA.bowtie2.mapq20.bam"),
    },
    # (add_chr) Chromosome relabeling (add "chr") and filtering (removing non-canonical chromosomes)
    "add_chr_dna": {
        "parent": "bowtie2_dna",
        "base": "dpm",
        "path": os.path.join("alignments_parts", "{sample}*.DNA.chr.bam"),
    },
    # (repeat_mask)
    "mask": {
        "parent": "add_chr_dna",
        "base": "dpm",
        "path": os.path.join("alignments_parts", "{sample}*.DNA.chr.masked.bam"),
    },
    # (merge_dna) -- final DNA read count; pipeline counting stops here
    "merge_dna": {
        "parent": "mask",
        "base": "dpm",
        "path": os.path.join("alignments", "{sample}.DNA.merged.bam"),
    },
    ## RNA processing ##
    # (cutadapt_rpm) Remove RPM from read1 of RPM reads, remove empty inserts
    "rpm_trim": {
        "parent": "rpm",
        "base": "rpm",
        "path": os.path.join("trimmed", "{sample}*_R1*.barcoded_rpm.RDtrim.fastq.gz"),
    },
    # (bowtie2_align) Align to repeat transcriptome
    "bowtie2_rna": {
        "parent": "rpm_trim",
        "base": "rpm",
        "path": os.path.join("alignments_parts", "{sample}*.bowtie2.mapped.bam"),
    },
    "bowtie2_rna_unaligned": {
        "parent": "rpm_trim",
        "base": "rpm",
        "path": os.path.join("alignments_parts", "{sample}*.bowtie2.unmapped.bam"),
    },
    # (bam_to_fastq) Convert unaligned reads to fastq
    "bam_to_fastq": {
        "parent": "bowtie2_rna_unaligned",
        "base": "rpm",
        "path": os.path.join("fastqs", "{sample}*.bowtie2.unmapped_R1.fq.gz")
    },
    # (star_align) Align to transcriptome
    "star_align" : {
        "parent": "bam_to_fastq",
        "base": "rpm",
        "path": os.path.join("alignments_parts", "{sample}*.Aligned.sortedByCoord.out.bam"),
    },
    # (add_chr_rna) Chromosome relabeling (add "chr") and filtering (removing non-canonical chromosomes)
    "add_chr_rna": {
        "parent": "star_align",
        "base": "rpm",
        "path": os.path.join("alignments_parts", "{sample}*.Aligned.sortedByCoord.out.chr.bam"),
    },
    # (merge_rna) -- combines directly-mapped reads (bowtie2_rna) with
    # re-aligned reads that went through star_align (add_chr_rna);
    # final RNA read count, pipeline counting stops here
    "merge_rna": {
        "parent": ["bowtie2_rna", "add_chr_rna"],
        "base": "rpm",
        "path": os.path.join("alignments", "{sample}.RNA.merged.bam"),
    },
    ## Oligo processing ##
    # (fastq_to_bam)
    "bpm_bam": {
        "parent": "bpm",
        "base": "bpm",
        "path": os.path.join("alignments_parts", "{sample}*.BPM.bam"),
    },
    # (merge_beads)
    "merge_beads": {
        "parent": "bpm_bam",
        "base": "bpm",
        "path": os.path.join("alignments", "{sample}.merged.BPM.bam"),
    },
}

PIPELINE_CLUSTERS = {
    "cluster": {
        "parent": {"dpm": "mask", "bpm": "bpm_bam", "rpm": ["add_chr_rna", "bowtie2_rna"]},
        "path": os.path.join("clusters_parts", "{sample}*.clusters"),
    },
    "merge_clusters": {
        "parent": {"dpm": "cluster", "bpm": "cluster", "rpm": "cluster"},
        "path": os.path.join("clusters", "{sample}.clusters"),
    },
}


class Node:
    """
    A node represents one step in the ChIP-DIP pipeline for a given sample.

    Most nodes have a single parent (the pipeline step that feeds into them),
    but some nodes (e.g. merge_rna, which combines directly-mapped bowtie2
    reads with re-aligned/star-mapped reads) are fed by more than one parent.
    """

    def __init__(
        self,
        sample,
        level,
        graph,
        children=None,
        parent=None,
        base=None,
        n_reads=np.nan,
        depth=None,
    ):
        """
        Args
        - sample: str
        - level: str
        - graph: Graph
        - children: set of Node or None
        - parent: Node, list/tuple/set of Node, or None
        - base: Node or None
        - n_reads: int or np.nan
        - depth: int or None
        """
        if children is None:
            children = set()
        self.sample = sample
        self.level = level
        self.name = f"{sample}_{level}"
        self.graph = graph
        self.children = set()
        self.parents = []
        self.add_children(children)
        self.set_parents(parent)
        self.base = base
        self.n_reads = n_reads
        self.depth = depth
        self.graph.add_node(self)

    def add_parent(self, parent):
        """Register `parent` as a parent of this node (and this node as one of
        `parent`'s children). No-op if `parent` is None or already registered."""
        if parent is None:
            return
        assert parent.graph == self.graph
        if parent not in self.parents:
            self.parents.append(parent)
        parent.children.add(self)

    def add_children(self, children):
        """Register each node in `children` as a child of this node (and this
        node as their parent)."""
        self.children |= set(children)
        for child in children:
            assert child.graph == self.graph
            child.add_parent(self)

    def set_parents(self, parents):
        """
        Args
        - parents: Node, list/tuple/set of Node, or None
        """
        if parents is None:
            parents = []
        elif isinstance(parents, Node):
            parents = [parents]
        for parent in parents:
            self.add_parent(parent)

    def get_root(self):
        """Walk up the first-parent chain and return the root node (the node
        with no parents) that this node ultimately descends from."""
        node = self
        while node.parents:
            node = node.parents[0]
        return node

    def __repr__(self):
        repr_parent = (
            ";".join(parent.name for parent in self.parents) if self.parents else "None"
        )
        repr_base = self.base.name if self.base is not None else "None"
        repr_children = (
            ";".join((child.name for child in self.children))
            if len(self.children) > 0
            else self.children
        )
        return f"{self.name} ({self.n_reads} reads, parent={repr_parent}, base={repr_base}, depth={self.depth}, children={repr_children})"

    def __hash__(self):
        return hash(self.name)

    def to_dict(self):
        """
        Summarize this node's read counts and where it sits in the pipeline
        graph, including its read count as a fraction of its parent(s), of its
        `base` node (the node this one is normalized against), and of the
        overall root node of its lineage.

        Returns
        - d: dict
            Keys: sample, depth, level, parent_level, base_level, root_level,
            n_reads, frac_parent, frac_base, frac_root.
        """
        if self.depth is None:
            self.graph.compute_depths()
        frac_root = None
        root = self.get_root()
        if self.parents:
            frac_root = self.n_reads / root.n_reads if root.n_reads > 0 else np.nan
        frac_base = None
        if self.base:
            frac_base = self.n_reads / self.base.n_reads if self.base.n_reads > 0 else np.nan
        frac_parent = None
        if self.parents:
            n_reads_parents = sum(parent.n_reads for parent in self.parents)
            frac_parent = self.n_reads / n_reads_parents if n_reads_parents > 0 else np.nan
        d = {
            "sample": self.sample,
            "depth": self.depth,
            "level": self.level,
            "parent_level": (
                ";".join(parent.level for parent in self.parents) if self.parents else None
            ),
            "base_level": self.base.level if self.base is not None else None,
            "root_level": root.level if self.parents else None,
            "n_reads": self.n_reads,
            "frac_parent": frac_parent,
            "frac_base": frac_base,
            "frac_root": frac_root,
        }
        return d

    def print_efficiency(self, sep="\t", sample=None, toprint=True):
        """
        Format (and by default print) a one-line, indented summary of this
        node's read count and its percentage relative to its parent, base, and
        root nodes.

        Args
        - sep: str
            Field separator.
        - sample: bool or None
            Whether to show the full sample name (self.name) instead of just
            the pipeline level (self.level). If None, defaults to True for
            root nodes (depth 0) and False otherwise.
        - toprint: bool
            Whether to print the line in addition to returning it.

        Returns
        - str_efficiency: str
            The formatted line.
        """
        d = self.to_dict()
        indent = ""
        if self.depth > 0:
            indent = "  " * (self.depth - 1) + "- "
        else:
            if sample is None:
                sample = True
        str_percent_root = (
            f"({100*d['frac_root']:.3g}% of {d['root_level']})"
            if d["frac_root"] is not None
            else ""
        )
        str_percent_base = (
            f"({100*d['frac_base']:.3g}% of {d['base_level']})"
            if d["frac_base"] is not None
            else ""
        )
        str_percent_parent = (
            f"({100*d['frac_parent']:.3g}% of {d['parent_level']})"
            if d["frac_parent"] is not None
            else ""
        )
        percents = sep.join((str_percent_parent, str_percent_base, str_percent_root)).rstrip()

        name = self.name if sample else self.level

        str_efficiency = (
            indent
            + name
            + sep
            + f"{int(self.n_reads) if not np.isnan(self.n_reads) else self.n_reads} reads"
            + ((sep + percents) if percents != "" else "")
        )
        if toprint:
            print(str_efficiency)
        return str_efficiency


class Graph:
    """
    A graph represents a bead-barcoding pipeline.
    """

    def __init__(self, nodes=None):
        if nodes is None:
            nodes = set()
        self.nodes = nodes
        for node in self.nodes:
            assert node.graph in (None, self)
            node.graph = self

    def add_node(self, node):
        """Register `node` in this graph (no-op if already present). Asserts
        no other node with the same name already exists."""
        if node not in self.nodes:
            assert node.name not in [n.name for n in self.nodes]
            self.nodes.add(node)
            assert node.graph in (None, self)
            node.graph = self

    def get_node_levels(self):
        """Return the set of distinct pipeline levels (e.g. "split_fastq",
        "trim", ...) present across all nodes in this graph."""
        return set([node.level for node in self.nodes])

    def get_node(self, sample, level):
        """Return the single node matching `sample` and `level`. Raises
        IndexError if no such node exists."""
        return [node for node in self.nodes if node.sample == sample and node.level == level][0]

    def get_roots(self):
        """Return all nodes with no parents (the entry points of the pipeline,
        e.g. split_fastq for each sample)."""
        return [node for node in self.nodes if not node.parents]

    def topological_order(self):
        """
        Return all nodes in topological order (each node appears after all
        of its parents). Required because some nodes (e.g. merge_rna) have
        multiple parents, so a simple DFS from the roots is not guaranteed
        to visit a node only after all of its parents have been visited.
        """
        remaining_parents = {node: len(node.parents) for node in self.nodes}
        order = [node for node in self.nodes if remaining_parents[node] == 0]
        i = 0
        while i < len(order):
            node = order[i]
            i += 1
            for child in node.children:
                remaining_parents[child] -= 1
                if remaining_parents[child] == 0:
                    order.append(child)
        assert len(order) == len(self.nodes), (
            "Could not compute a topological order for all nodes; check for "
            "cycles or parents that reference nodes outside this graph."
        )
        return order

    def compute_depths(self):
        """Set `node.depth` for every node in the graph: a root node has depth
        0, and every other node's depth is one more than its deepest parent."""
        for node in self.topological_order():
            node.depth = max((parent.depth for parent in node.parents), default=-1) + 1

    def to_df(self, node=None, dicts=None, visited=None):
        """
        Recursively (DFS from the roots) collect every node's `to_dict()` into
        a single DataFrame, one row per node.

        Args
        - node, dicts, visited: internal recursion state; leave as None when
          calling from outside the class.

        Returns
        - pd.DataFrame with one row per node (columns per Node.to_dict()).
        """
        if dicts is None:
            dicts = []
        if visited is None:
            visited = set()
        if node is None:
            next_nodes = self.get_roots()
        else:
            assert node in self.nodes
            # A node may be reached more than once via DFS if it has multiple
            # parents (e.g. merge_rna); only record it once.
            if node not in visited:
                visited.add(node)
                dicts.append(node.to_dict())
            next_nodes = node.children
        for next_node in next_nodes:
            self.to_df(node=next_node, dicts=dicts, visited=visited)
        return pd.DataFrame(dicts)

    def print_efficiency(self, node=None, history=None, visited=None, **kwargs):
        """
        Recursively (DFS from the roots) print (by default) every node's
        `print_efficiency()` line, in traversal order.

        Args
        - node, history, visited: internal recursion state; leave as None
          when calling from outside the class.
        - **kwargs: forwarded to each Node.print_efficiency() call.

        Returns
        - history: list of str
            The formatted line for every node, in the order printed.
        """
        if history is None:
            history = []
        if visited is None:
            visited = set()
        if node is None:
            next_nodes = self.get_roots()
        else:
            assert node in self.nodes
            # A node may be reached more than once via DFS if it has multiple
            # parents (e.g. merge_rna); only print it once.
            if node not in visited:
                visited.add(node)
                history.append(node.print_efficiency(**kwargs))
            next_nodes = node.children
        for next_node in next_nodes:
            self.print_efficiency(node=next_node, history=history, visited=visited)
        return history

    def merge_samples(self):
        """
        Merge all sample sub-graphs into a single Graph with sample="all",
        summing n_reads for nodes sharing the same level across samples.

        Nodes are processed in topological order (rather than via recursive
        DFS from the roots) so that a node with multiple parents (e.g.
        merge_rna, fed by both bowtie2_rna and add_chr_rna) is only ever
        merged in after both of its parents already exist in G_merged.
        """
        G_merged = Graph()
        for node in self.topological_order():
            node_merge_parents = [
                G_merged.get_node("all", parent.level) for parent in node.parents
            ]
            node_base_level = None if node.base is None else node.base.level
            node_merge_base = (
                G_merged.get_node("all", node_base_level) if node_base_level else None
            )
            if node.level not in G_merged.get_node_levels():
                node_merge = Node(
                    "all",
                    node.level,
                    G_merged,
                    parent=node_merge_parents if node_merge_parents else None,
                    base=node_merge_base,
                    n_reads=0,
                )
            else:
                node_merge = G_merged.get_node("all", node.level)
                assert set(node_merge.parents) == set(node_merge_parents)
                assert node_merge.base is node_merge_base
            # Each (sample, level) node is visited exactly once in the
            # topological order, so no separate de-duplication is needed here.
            node_merge.n_reads += node.n_reads
        return G_merged


def count_lines(paths, n_processes=1, cmd_unzip=None, cmd_count=None, env=None, return_raw=False):
    """
    Count the number of lines in each file.

    Args
    - paths: list-like
        Paths to read files. All read files must be of the same file type.
    - n_processes: int. default=1
        Number of parallel processes to use.
    - cmd_unzip: str. default=None
        Command to write file contents to standard output.
        If None, defaults to 'cat'.
        Example: If pigz is available, use 'unpigz -c' instead of 'gunzip -c' for a
          small speedup for gzipped files.
    - cmd_count: str. default=None
        Command that reads from standard in lines output from cmd_unzip and
        outputs an integer.
        If None, defaults to 'wc -l'.
    - env: dict. default=None
        Environment variables for shell commands.
    - return_raw: bool. default=False
        Return unprocessed output as split lines from underlying counting subprocesses.

    Returns
    - If return_raw is True: list of str, where each element is '<path><tab><count>'
    - If return_raw is False: dict[str, int], where keys are paths and values are counts
    """
    if cmd_unzip is None:
        cmd_unzip = "cat"
    if cmd_count is None:
        cmd_count = "wc -l"
    cmd_xargs = f"xargs -n 1 -P {n_processes} sh -c"
    cmd_shell = f'{cmd_unzip} "$0" | {cmd_count} | awk -v pre="$0" ' + "'$0=pre\"\t\"$0'"
    cmd = shlex.split(cmd_xargs) + [cmd_shell]

    # The equivalent terminal command looks something like
    #
    # ls path1 path2 | xargs -n 1 -P n_processes sh -c $'cat "$0" | wc -l | awk -v pre="$0" \'$0=pre"\t"$0\''
    #
    # where the construction sh -c $'... \' ...' is used to escape the internal single quote while maintaining
    # the entire command as a single string.
    #
    # The idea is to pass each path to the following shell command:
    #
    # cat "$0" | wc -l | awk -v pre="$0" '$0=pre"\t"$0'
    #
    # where the shell variable "$0" is the path of the input file.
    # The purpose of the awk command is to output the string <path><tab><count>
    # - The shell variable "$0" is passed into awk as a variable of name "pre".
    # - The awk variable "$0" is the input line piped in from from wc -l, which is just the count.
    # - The awk script therefore appends <pre><tab> before the count.

    with subprocess.Popen(["ls"] + paths, stdout=subprocess.PIPE, env=env) as p1:
        with subprocess.Popen(cmd, stdin=p1.stdout, stdout=subprocess.PIPE, env=env) as p2:
            p1.stdout.close()
            out = p2.communicate()[0].decode().strip().splitlines()
    if return_raw:
        return out
    counts = {path: int(count) for path, count in map(lambda s: s.strip().rsplit("\t", 1), out)}
    return counts


def count_reads(
    paths, filetype=None, cmd_unzip=None, n_processes=None, agg=True, return_raw=False, **kwargs
):
    """
    Count the number of reads in FASTQ or SAM/BAM/CRAM files.

    Assumes samtools is available.

    Args
    - paths: list-like
        Paths to read files. All read files must be of the same file type.
    - filetype: str. default=None
        Type of files to count.
        - 'fastq': files ending in .fastq, .fq., .fastq.gz, or .fq.gz
        - 'bam': files ending in .sam, .bam., or .cram
        - None: automatically detect based on the first file in paths
    - cmd_unzip: str. default=None
        Command to write file contents to standard output. See count_lines().
    - n_processes: int. default=None
        Number of parallel processes to use.
        If None, defaults to 1.
    - agg: bool. default=True
        Return sum of read counts
    - return_raw: bool. default=False
        Return unprocessed output as split lines from underlying counting subprocesses.
    - **kwargs
        Additional keyword arguments to count_lines()
        - cmd_count
        - env

    Returns: depends
    - If return_raw is True: returns list(str)
    - If agg is False: returns dict(str -> int), giving the read counts for each path in paths
    - If agg is True: returns int, the sum of read counts over all files in paths
    """
    if n_processes is None:
        n_processes = 1
    if filetype is None:
        if regex_fastq.search(paths[0]):
            assert all(map(regex_fastq.search, paths)), "Files with differing extensions detected."
            filetype = "fastq"
        elif regex_bam.search(paths[0]):
            assert all(map(regex_bam.search, paths)), "Files with differing extensions detected."
            filetype = "bam"
        else:
            raise ValueError(
                (
                    f"Unsupported file extension for file {paths[0]}. "
                    "Must be .fastq.gz, .fq.gz, .bam, .sam., or .cram"
                )
            )
    assert str(filetype).lower() in ("fastq", "bam")
    factor = 4 if filetype == "fastq" else 1

    if cmd_unzip is None:
        if paths[0].endswith(".gz"):
            assert all(
                (path.endswith(".gz") for path in paths)
            ), "Files with different compressions detected."
            if shutil.which("unpigz"):
                cmd_unzip = "unpigz -c"
            else:
                cmd_unzip = "gunzip -c"
        elif filetype == "bam":
            cmd_unzip = f"samtools view -@ {int(max(n_processes / len(paths), 1))}"
    counts = count_lines(paths, cmd_unzip=cmd_unzip, return_raw=return_raw, **kwargs)
    if return_raw:
        return counts
    if factor != 1:
        counts = {path: count / factor for path, count in counts.items()}
    return sum(counts.values()) if agg else counts


def count_reads_clusters(paths, n_processes):
    """
    Count the number of DPM, RPM and BPM reads in SPRITE cluster files.
    Implemented as a wrapper around helpers.count_reads()

    Args
    - paths: list-like
        Paths to cluster files
    - n_processes: int
        Number of parallel processes to use.

    Returns: dict
        Keys = 'DPM', 'BPM', 'RPM'
        Values = sum of the number of DPM or RPM or  BPM reads in cluster files.
    """
    counts = dict(DPM=0, BPM=0, RPM=0)
    out = count_lines(
        paths,
        n_processes=n_processes,
        cmd_unzip="cat",
        cmd_count=f'grep -h -F -o -e "DPM" -e "BPM" -e "RPM" | sort --parallel {n_processes} | uniq -c',
        return_raw=True,
    )
    for line in out:
        count, pattern = regex_uniq_count.match(line.strip().rsplit("\t", 1)[1].strip()).groups()
        counts[pattern] += int(count)
    return counts


def collect_pipeline_counts(
    samples,
    DIR_WORKUP,
    n_processes,
    pipeline=None,
    pipeline_clusters=None,
    unzip=None,
    verbose=True,
):
    """
    Parse pipeline read file counts into a Graph.

    Args
    - samples: list-like of str
    - DIR_WORKUP: str
        Directory to which all paths given in pipeline or pipeline_clusters are relative to
    - n_processes: int
        Number of processes
    - pipeline: dict. default=None
        Pipeline specification. If None, defaults to PIPELINE.
    - pipeline_clusters: dict. default=None
        Pipeline specification for cluster files. If None, defaults to PIPELINE_CLUSTERS.
    - unzip: str. default=None
        Command (e.g., 'zcat', 'unpigz -c') to write file contents to standard output.
        See helpers.count_reads() docstring.
    - verbose: bool. default=True
        Print status messages to standard error.

    Returns: tuple(Graph, Graph or None)
    - G: Graph
        Graph with potentially multiple root nodes, each corresponding to an sample
    - G_merged: Graph or None
        - If the number of samples is 1, G_merged is None
        - Otherwise, a Graph where all the samples in G have been merged; one root node
    """
    if pipeline is None:
        pipeline = PIPELINE
    if pipeline_clusters is None:
        pipeline_clusters = PIPELINE_CLUSTERS
    G = Graph()

    for level, node_info in pipeline.items():
        if verbose:
            print(f"Counting {level}", file=sys.stderr)
        for sample in samples:
            paths = glob.glob(os.path.join(DIR_WORKUP, node_info["path"].format(sample=sample)))
            if len(paths) == 0:
                count = 0
            else:
                cmd_unzip = None
                if node_info["path"].endswith(".gz"):
                    cmd_unzip = unzip
                count = count_reads(paths, n_processes=n_processes, cmd_unzip=cmd_unzip)
            base = None
            if "base" in node_info and node_info["base"]:
                base = G.get_node(sample, node_info["base"])
            parent = None
            if node_info["parent"]:
                if isinstance(node_info["parent"], (list, tuple)):
                    parent = [G.get_node(sample, p) for p in node_info["parent"]]
                else:
                    parent = G.get_node(sample, node_info["parent"])
            node = Node(sample, level, graph=G, n_reads=count, parent=parent, base=base)
    for level, node_info in pipeline_clusters.items():
        if verbose:
            print(f"Counting {level}", file=sys.stderr)
        for sample in samples:
            paths = glob.glob(os.path.join(DIR_WORKUP, node_info["path"].format(sample=sample)))
            if len(paths) == 0:
                counts = dict(DPM=0, BPM=0, RPM=0)
            else:
                counts = count_reads_clusters(paths, n_processes=n_processes)
            for base_level, parent in node_info["parent"].items():
                if isinstance(parent, (list, tuple)):
                    parent_levels = [
                        p + f"-{base_level}" if p == "cluster" else p for p in parent
                    ]
                    parent_nodes = [G.get_node(sample, p) for p in parent_levels]
                else:
                    if parent == "cluster":
                        parent += f"-{base_level}"
                    parent_nodes = G.get_node(sample, parent)
                node = Node(
                    sample,
                    f"{level}-{base_level}",
                    graph=G,
                    n_reads=counts[base_level.upper()],
                    parent=parent_nodes,
                    base=G.get_node(sample, base_level),
                )
    G_merged = None
    if len(samples) > 1:
        G_merged = G.merge_samples()
    return G, G_merged


def main():
    """CLI entry point: parse arguments, build the pipeline count graph(s) via
    collect_pipeline_counts(), print a per-sample (and merged, if applicable)
    efficiency summary, and optionally save the counts to a CSV file."""
    parser = argparse.ArgumentParser(
        description="Count reads at each step of the bead-barcoding pipeline."
    )
    parser.add_argument("--samples", required=True, help="path to samples.json file")
    parser.add_argument("-w", "--dir-workup", required=True, help="path to workup directory")
    parser.add_argument("-n", "--n-processes", type=int, help="number of processes to use")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="print status messages to stderr"
    )
    parser.add_argument("-o", "--out", help="path to save counts as CSV file")
    parser.add_argument(
        "-u",
        "--unzip",
        help="command to write gzip-compressed file contents to standard output: e.g., zcat, unpigz -c, etc.",
    )
    parser.add_argument(
        "-s", "--sep", default="\t", help='separator for printing efficiency output (default="\\t")'
    )

    args = parser.parse_args()
    n_processes = args.n_processes
    if n_processes is None:
        try:
            n_processes = len(os.sched_getaffinity(0))
        except AttributeError:
            n_processes = os.cpu_count()
    DIR_WORKUP = args.dir_workup

    with open(args.samples, "rt") as f:
        samples = tuple(json.load(f).keys())
    G, G_merged = collect_pipeline_counts(
        samples, DIR_WORKUP, n_processes, unzip=args.unzip, verbose=args.verbose
    )

    if args.verbose:
        print("Pipline Read Counts Per Sample:", file=sys.stderr)
    G.print_efficiency(sep=args.sep)
    if G_merged:
        print()
        if args.verbose:
            print("Pipline Read Counts Aggregated Over All Samples", file=sys.stderr)
        G_merged.print_efficiency(sample=False, sep=args.sep)
    if args.out:
        df = G.to_df()
        if G_merged:
            df = pd.concat((df, G_merged.to_df()), axis=0, ignore_index=True)
        df.to_csv(args.out)


if __name__ == "__main__":
    main()
