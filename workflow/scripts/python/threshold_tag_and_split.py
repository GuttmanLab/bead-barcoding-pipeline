...
def main():
    """CLI entry point: assign antibody labels to clusters, apply those labels
    to the input BAM as RG tags (deduplicating along the way), then split the
    labeled BAM into one file per antibody."""
    args = parse_args()
...
def label_bam_file(input_bam, output_bam, labels, num_tags, progress=False):
    ...
    with pysam.AlignmentFile(input_bam, "rb") as in_bam, \
        ...
        for read in tqdm(in_bam.fetch(until_eof=True), disable=not progress):
            ...
            # the read-type tag ("DPM", "RPM", or "BEAD_...") is always the
            # first bracketed group right after "::"; drop it before joining
            # the remaining split-pool tags into the cluster barcode
            barcode_drop = barcode[1:]
            barcode_drop.append(library)
            full_barcode = ".".join(barcode_drop)
