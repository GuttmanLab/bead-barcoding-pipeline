...
def filter_reads(
    ...
    with pysam.AlignmentFile(path_bam_in, "rb", threads=threads) as file_bam_in:
        ...
        def process_reads(output_stream):
            """Write each read in file_bam_in to output_stream if its
            reference is in chrom_map (remapping its reference IDs to match
            the new header), discarding reads on chromosomes not in the map."""
