"""
Convert a BPM (bead oligo) FASTQ file into a BAM file, encoding each read's
UMI as its reference start position and its target (antibody/bead) identity
as its reference name.
"""
...
def main():
    """Read the BPM FASTQ input, build a BAM header from the barcode config, and
    write one BAM record per read."""
...
def construct_sam_header(config):
    ...
    reference_names = [x.replace("BEAD_", "") for x in df["ids"] if "BEAD_" in x]
...
def sequence_to_int(seq, query_name):
    """
    Encode a nucleotide sequence as an integer by mapping each base to a digit
    (A=1, C=2, G=3, T=4, N=0) and concatenating the digits, then parsing the
    result as a base-10 integer. Used to pack a UMI sequence into a BAM record's
    reference_start field so identical UMIs can be compared/deduplicated as
    plain integers downstream.

    Args
    - seq: str
        Nucleotide sequence to encode (typically just the UMI portion of a read).
    - query_name: str
        Read name, used only for the error message if encoding fails.

    Returns
    - value: int
        The encoded UMI, or 0 if `seq` contains characters outside ACGTN.

    Note: since the encoding just concatenates digit strings and parses as an
    integer, UMIs that differ only by a leading N (which encodes to "0") can
    collide, e.g. "AC" -> "12" -> 12 and "NAC" -> "012" -> 12. Pre-existing
